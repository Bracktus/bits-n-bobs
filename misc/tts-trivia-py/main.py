...
norm = lambda col : [v/255 for v in col]
BLUE = norm([53, 79, 157])
...
def read_qas(filename="Trivia.xlsx"):
    ...
    questions = [list(map(str, question)) for idx, question in enumerate(rows) if idx % 2 == 0]
    answers = [list(map(str, answer)) for idx, answer in enumerate(rows) if idx % 2 == 1]
...
def draw_card(card, type, card_name):
    ...
    # font settings
    ctx.select_font_face("Open Sans", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
